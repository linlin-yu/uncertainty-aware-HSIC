...
from gpn_misclassification_cleangraph import tune_gpn_mis
import pandas as pd
...
def tune_main(config, num_samples=5, gpus_per_trial=1, save_dir='tune'):
    ...
    analysis = tune.run(
        ...
        metric="val_evaluation/val_id_ce",
        mode="min",
        config=config,
...
# config['model']['radial_layers'] = tune.grid_search([16])
...
suffix = 'lrwder-redo'
config['model']['entropy_reg_weight'] = tune.grid_search([0.1, 0.01, 0.001, 0.0001, 0.00001])
config['training']['weight_decay'] = tune.grid_search([0.00001, 0.0005, 0.0001, 0.001, 0.005, 0.001])
...
# config['model']['entropy_reg_weight'] = 0.001
# config['training']['weight_decay'] = 0.005
# config['training']['lr'] = 0.001
# config['model']['reconstruction_reg_weight'] = tune.grid_search([0.1, 0.01, 0.001, 0.0001, 0])
# # config['model']['tv_alpha_reg_weight'] = tune.grid_search([0.1, 0.01, 0.001, 0.0001, 0])
# config['model']['tv_vacuity_reg_weight'] = tune.grid_search([0.1, 0.01, 0.001, 0.0001, 0])
